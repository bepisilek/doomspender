...
const App = {
  ...
  init() {
    ...
    this.switchTab(this.state.view || "calc");
  ...
  bindEvents() {
    ...
    $all("[data-close='profile']", this.el.profileModal).forEach((btn) => {
    ...
    if (this.el.calcForm) {
      ...
      $all("input", this.el.calcForm).forEach((input) => {
    ...
    if (this.el.goalForm) {
      this.el.goalForm.addEventListener("submit", (event) => {
        event.preventDefault();
        this.saveGoals(new FormData(this.el.goalForm));
      });
    }
    ...
    $all('[data-tab-target="calc"]').forEach((btn) => {
      btn.addEventListener("click", () => this.switchTab("calc"));
    });
  },
  ...
  showApp() {
    if (this.el.welcomeScreen) {
      this.el.welcomeScreen.classList.add("hidden");
    }
    if (this.el.appScreen) {
      this.el.appScreen.classList.remove("hidden");
    }
    this.switchTab(this.state.view || "calc");
  },
  ...
  resetAllData() {
    ...
    this.state.view = "calc";
    if (this.el.appScreen) {
      this.el.appScreen.classList.add("hidden");
    }
    if (this.el.welcomeScreen) {
    ...
    if (this.el.calcResult) {
      this.el.calcResult.classList.add("hidden");
    }
    if (this.el.welcomeHint) {
  ...
  renderStats() {
    ...
    if (this.el.statsEmpty) {
      this.el.statsEmpty.classList.add("hidden");
    }
  ...
  populateGoalsForm() {
    if (!this.el.goalForm) return;
    this.el.goalForm.monthlyCap.value = this.state.goals.monthlyCap || "";
    this.el.goalForm.savingGoal.value = this.state.goals.savingGoal || "";
  },
  ...
  switchTab(view) {
    ...
    const fallback = tabs.length ? tabs[0].dataset.tab : "calc";
    const target = tabs.find((tab) => tab.dataset.tab === view)?.dataset.tab || fallback;
    ...
    this.el.views.forEach((section) => {
      const active = section.dataset.tabview === target;
